...
export default function VisitForm() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.visitDate || !formData["สาขา"] || !formData.problem.trim() || !formData.resolution_text.trim()) {
    ...
    try {
      ...
      if (result.success) {
        ...
        setFormData({
          visitDate: "", reason: "", problem: "", resolution_text: "", สาขา: "",
      ...
      <div style={styles.contentWrap}>
        <div style={styles.heroCard}>
          <div style={styles.heroContent}>
            ...
            <p style={styles.pageSubtitle}>ระบบบันทึกรายงานการเข้าตรวจงาน กรุณาระบุข้อมูลและรูปภาพเพื่อประสิทธิภาพในการตรวจสอบ</p>
          </div>
          <div style={styles.topActions}>
            {currentUser?.role === "admin" && (
              <button type="button" style={styles.reportBtn} onClick={() => navigate("/report")}>ดูรายงาน</button>
            )}
            <button type="button" style={styles.logoutBtn} onClick={handleLogout}>ออกจากระบบ</button>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} style={styles.formGrid}>
          <div style={styles.sectionCard}>
            ...
            <div style={styles.fieldWrap}>
              ...
              <input type="text" name="reason" value={formData.reason} onChange={handleChange} placeholder="เช่น เข้าตรวจเช็คระบบการทำงาน" style={styles.input} />
            </div>
              
            <div style={styles.fieldWrap}>
              <label style={styles.label}>รายละเอียดและปัญหาที่ตรวจพบ <span style={styles.required}>*</span></label>
              <textarea name="problem" value={formData.problem} onChange={handleChange} placeholder="โปรดอธิบายสถานการณ์หรือปัญหาที่พบเจอดโดยละเอียด..." style={styles.textarea} />
            </div>
            ...
            <div style={styles.grid2}>
              <div style={styles.uploadArea}>
                <div style={styles.uploadHeader}>
                  ...
                  <label style={styles.uploadTrigger}>
                    เพิ่มรูป
                    <input type="file" accept="image/*" multiple onChange={(e) => handleFileChange(e, "beforeImages")} style={styles.hiddenInput} />
                  </label>
                </div>
                <div style={styles.previewContainer}>
                  {formData.beforeImages.map((file, i) => (
                    <div key={i} style={styles.imageCard}>
                      ...
                      <button type="button" style={styles.deleteIcon} onClick={() => removeFile("beforeImages", i)}>✕</button>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={styles.uploadArea}>
                <div style={styles.uploadHeader}>
                  ...
                  <label style={styles.uploadTrigger}>
                    เพิ่มรูป
                    <input type="file" accept="image/*" multiple onChange={(e) => handleFileChange(e, "afterImages")} style={styles.hiddenInput} />
                  </label>
                </div>
                <div style={styles.previewContainer}>
                  {formData.afterImages.map((file, i) => (
                    <div key={i} style={styles.imageCard}>
                      <img src={file.preview} alt="" style={styles.imageThumb} />
                      <button type="button" style={styles.deleteIcon} onClick={() => removeFile("afterImages", i)}>✕</button>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div style={styles.submitSection}>
            <button type="submit" style={isSubmitting ? styles.submitBtnDisabled : styles.submitBtn} disabled={isSubmitting}>
